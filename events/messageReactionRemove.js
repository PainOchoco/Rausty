  ...
  if (reaction.message.channel.id === rChannel.id) {
    let applyRole = async () => {
      ...
      try {
        switch (emojiName) {
          case "🖥":
            member.roles.remove(rInformatique);
            break;
          case "🛠":
            member.roles.remove(rMakers);
            break;
          case "🐧":
            member.roles.remove(rLinuxMaster);
            break;
          case "🐙":
            member.roles.remove(rGithubMaster);
            break;
          case "💡":
            member.roles.remove(rPingCobalt);
            break;
        }
      } catch (err) {
        console.log(err);
